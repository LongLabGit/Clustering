...
for c = 1:numel(clusterIDs)
  ...
  [~, bestFeatures] = sort(mean(fmask(clu == clusterIDs(c), :)), 'descend'); % sorting the features by the masks (the higher the mask values, the better a feature is for a given cluster)
