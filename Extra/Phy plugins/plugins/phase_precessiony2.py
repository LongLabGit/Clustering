...
class PhasePrecession2(IPlugin):
    def attach_to_controller(self, controller):
        def _load_positionTime(self):
            return self._read_array('posTime')

        def _load_posLin2(self):
            return self._read_array('posLin2')
        ...
        posLin2 = _load_posLin2(controller.model)
        set_interp3 = interp1d(time_pos, posLin2, kind='linear', bounds_error=False, fill_value=np.nan, assume_sorted=False)
        ...
        def on_gui_ready(gui):
            ...
            gui.add_view(f, name='PhasePrecession Central arm')
