...
cd ../../
ops.root='S:\Vigi\NeuroGrid\Data\338A\Recording\Session1_Denoised\';
ops.fbinary=[ops.root,'Session1_ica3.dat'];
ops.fproc=[ops.root,'temp_wh.dat'];
ops.datatype            = 'dat';  % binary ('dat', 'bin') or 'openEphys'
ops.NchanTOT            = 120; % total number of channels
ops.fs                  = 20000; % sampling rate
ops.chanMap =  'Extra/ChanMaps/ChanMap120.mat';
load(ops.chanMap,'connected')
ops.Nchan               = sum(connected); % number of active channels 

%Some Settings to play with 
ops.Nfilt               = 192; %  number of filters to use (512, should be a multiple of 32)
ops.criterionNoiseChannels = .2; % fraction of "noise" templates allowed to span all channel groups (see createChannelMapFile for more info). 
%i.e. the number of shanks a template can span is either 1 or
%>ops.criterionNoiseChannels, but NOT in between. 
...
ops.Th               = [4 7 7];    % threshold for detecting spikes on template-filtered data ([6 12 12]) 
...
ops.fslow              = 6000;   % frequency for low pass filtering
...
ops.nNeighPC    = 4; %12; % number of channnels to mask the PCs, leave empty to skip (12)
...
ops.maskMaxChannels = 4;       % how many channels to mask up/down ([5]). like flood fill on KK2
