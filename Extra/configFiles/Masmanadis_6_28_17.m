...
cd ../../
ops.root='S:/Vigi/Datasets/SiliconProbe/Masmanadis/CutAmplifier/';
...
ops.chanMap =  'Extra/chanMaps/64H.mat';
...
ops.fslow              = 5000;   % frequency for high pass filtering
...
ops.nNeighPC    = 32; %12; % number of channnels to mask the PCs, leave empty to skip (12)
...
ops.maskMaxChannels = 10;       % how many channels to mask up/down ([5]). like flood fill on KK2
...
if ~exist(ops.root, 'dir')
    error('Wrong folder you shmuck')
elseif ~exist(batch_path, 'dir')
    mkdir(batch_path);
end
