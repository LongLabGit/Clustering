...
% fs=amplifier sample re;
% nC=number of channels in amplifier
% nA=number of analog inputs
% total_seconds=length of recording
...
if loadAll
    ...
    nC=length(amplifier_channels);
    nA=length(board_adc_channels);
    [~,rem]=strtok(fliplr(filename),'\');
    fileinfo = dir([fliplr(rem),'amplifier.dat']);
    total_seconds = fileinfo.bytes/(nC * 2); % int16 = 2 bytes
    total_seconds=total_seconds/fs;
end
% 
